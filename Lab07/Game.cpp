...
void Game::LoadData()
{
	...
	Matrix4 viewMatrix = Matrix4::CreateLookAt(Vector3(-300.0f, 0.0f, 0.0f), Vector3(20.0f, 0.0f, 0.0f), Vector3::UnitZ);
...
void Game::LoadBlocks(std::string filename, float spawnX) {
	//Open the file
	std::fstream file;
	file.open(filename);
	if (file.is_open()) {
		...
		while (!file.eof()) {
			...
			for (size_t i = 0; i < line.length(); i++) {
				...
				if (c == 'A' || c == 'B') {
					//Make a block
					Vector3 pos(spawnX, -LEVEL_SIZE/2 + ((float)i) * BLOCK_SIZE, LEVEL_SIZE/2 - ((float)j) * BLOCK_SIZE);
					Block* newBlock = new Block(this, c == 'B');
