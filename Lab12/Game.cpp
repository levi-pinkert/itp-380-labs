...
Game::Game()
:mIsRunning(true), mPlayer(nullptr)
{
...
void Game::UpdateGame()
{
	...
	//Increment the timer
	mTimer += deltaTime;
	if (mCheckpointTextTimer > 0.0f) {
		mCheckpointTextTimer = Math::Max(mCheckpointTextTimer - deltaTime, 0.0f);
		if (mCheckpointTextTimer == 0.0f) {
			SetCheckpointText(nullptr);
		}
	}
...
void Game::AddCheckpoint(Checkpoint* newCheckpoint) {
	if (mCheckpoints.empty()) {
		newCheckpoint->SetActive(true);
	}
	mCheckpoints.push(newCheckpoint);
}

void Game::RemoveCheckpoint() {
	mPlayer->SetRespawnPos(mCheckpoints.front()->GetPosition());
	mCheckpoints.front()->SetState(ActorState::Destroy);
	mCheckpoints.pop();
	if (!mCheckpoints.empty()) {
...
Checkpoint* Game::GetActiveCheckpoint() {
	return mCheckpoints.empty() ? nullptr : mCheckpoints.front();
}
...
void Game::SetCheckpointText(Texture* text) {
	if (mCheckpointText != nullptr) {
		mCheckpointText->Unload();
		mCheckpointText = nullptr;
	}
	if (text != nullptr) {
		mCheckpointText = text;
		mCheckpointTextTimer = CHECKPOINT_TEXT_TIME;
	}
}