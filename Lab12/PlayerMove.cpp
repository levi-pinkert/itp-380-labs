...
PlayerMove::PlayerMove(Actor* actor) : MoveComponent(actor) {
	...
	mRunSoundChannel = Mix_PlayChannel(-1, mOwner->GetGame()->GetSound("Assets/Sounds/Running.wav"), -1);
	Mix_Pause(mRunSoundChannel);
	ChangeState(Falling);
}

PlayerMove::~PlayerMove() {
	Mix_HaltChannel(mRunSoundChannel);
}
